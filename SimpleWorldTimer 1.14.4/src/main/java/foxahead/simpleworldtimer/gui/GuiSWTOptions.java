...
public class GuiSWTOptions extends Screen implements ISWTSliderObserver {
  ...
  public void init() {
    ...
    checkBoxEnable   = this.addButton(new GuiSWTCheckbox(xCoord,
                                                         ...
                                                         100,
                                                         20,
                                                         I18n.format("options.swt.enable"),
                                                         ConfigSWT.getEnable(),
                                                         (onPress11) -> {
    ...
    checkBoxAutoHide = this.addButton(new GuiSWTCheckbox(xCoord + 100,
                                                         ...
                                                         100,
                                                         20,
                                                         I18n.format("options.swt.autoHide"),
    ...
    smallButtonPreset       = this.addButton(new GuiSWTSmallButton(xCoord,
                                                                   ...
                                                                   176,
                                                                   20,
                                                                   I18n.format("options.swt.preset"),
                                                                   ConfigSWT.getPresetList(),
                                                                   ConfigSWT.getPreset(),
                                                                   (onPress21) -> {
                                                                                             this.actionPerformed(21);
                                                                                           }));
    buttonCustom            = this.addButton(new Button(xCoord + 180, yCoord + yStep * 4, 20, 20, "C", (onPress4) -> {
    ...
    this.startDateTextField = new TextFieldWidget(this.font, xCoord + 150, yCoord + yStep * 5, 50, 20, "");           //1
    startDateTextField.func_212954_a((onText1) -> {
      this.onTextField(1);
    });
    this.patternTextField1 = new TextFieldWidget(this.font, xCoord, yCoord + yStep * 6, 176, 20, ""); //2
    patternTextField1.func_212954_a((onText1) -> {
      this.onTextField(2);
    });
    this.patternTextField2 = new TextFieldWidget(this.font, xCoord, yCoord + yStep * 7, 176, 20, ""); //3
    patternTextField2.func_212954_a((onText1) -> {
      this.onTextField(3);
    });
    this.children.add(startDateTextField);
    this.children.add(patternTextField1);
    this.children.add(patternTextField2);
    buttonSwap = this.addButton(new Button(xCoord + 180, (int) (yCoord + yStep * 6.5F), 20, 20, ")", (onPress5) -> {
      this.actionPerformed(5);
    }));
    this.addButton(new Button(xCoord, yCoord + yStep * 8, 200, 20, I18n.format("gui.done"), (onPress0) -> {
      this.actionPerformed(0);
    }));
    buttonStart = this.addButton(new Button(xCoord + 102, yCoord + yStep * 5, 30, 20, ">", (onPress6) -> {
                  this.actionPerformed(6);
                }));
    buttonPause = this.addButton(new Button(xCoord + 136, yCoord + yStep * 5, 30, 20, "||", (onPress7) -> {
                  this.actionPerformed(7);
                }));
    buttonStop  = this.addButton(new Button(xCoord + 170, yCoord + yStep * 5, 30, 20, "[]", (onPress8) -> {
  ...
  public void render(int par1, int par2, float par3) {
    ...
    this.drawCenteredString(this.font, I18n.format("options.swt.title"), this.width / 2, yCoord, COLOR1);
    if (smallButtonClockType.getIndex() == ConfigSWT.CLOCK_TYPE_MINECRAFT) {
      this.drawCenteredString(this.font,
                              I18n.format("options.swt.startYear"),
                              xCoord + 120,
                              yCoord + yStep * 5 + 6,
                              COLOR2);
    }
