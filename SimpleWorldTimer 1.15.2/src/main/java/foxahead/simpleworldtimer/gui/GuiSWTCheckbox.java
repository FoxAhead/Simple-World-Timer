...
public class GuiSWTCheckbox extends Button {
  ...
  public GuiSWTCheckbox(int x, int y, String text, boolean state, Button.IPressable onPress) {
    this(x, y, 200, 20, text, state, onPress);
  }

  public GuiSWTCheckbox(int x, int y, int width, int height, String text, boolean state, Button.IPressable onPress) {
  ...
  @Override
  public void renderButton(int par2, int par3, float partialTicks) {
    if (this.visible) {
      ...
      if (this.State) {
        ...
        this.blit(this.x + 3, this.y + 3, 91, 223, 16, 16);
      }
      this.drawString(fontrenderer, this.getMessage(), this.x + 20 + 4, this.y + (this.height - 8) / 2, textColor);
    }
  }

  public void drawEmptyRect(int parX1, int parY1, int parX2, int parY2, int parColor) {
    hLine(parX1, parX2, parY1, parColor);
    hLine(parX1, parX2, parY2, parColor);
    vLine(parX1, parY1, parY2, parColor);
    vLine(parX2, parY1, parY2, parColor);
  }
}
