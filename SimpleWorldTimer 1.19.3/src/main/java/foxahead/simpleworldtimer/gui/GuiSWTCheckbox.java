...
public class GuiSWTCheckbox extends Button {
  ...
  public void renderButton(PoseStack PoseStack, int par2, int par3, float partialTicks) {
    if (this.visible) {
      ...
      this.isHovered = par2 >= getX() && par3 >= getY() && par2 < this.getX() + this.width && par3 < this.getY() + this.height;
      int textColor = (this.isHoveredOrFocused()) ? 0xFFFFFFA0 : 0xFFE0E0E0;
      int boxColor  = (this.isHoveredOrFocused()) ? 0xFFFFFFA0 : 0xFF000000;
      drawEmptyRect(PoseStack, this.getX() + 2, this.getY() + 2, this.getX() + 20 - 3, this.getY() + 20 - 3, boxColor);
      if (this.State) {
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        this.blit(PoseStack, this.getX() + 3, this.getY() + 3, 91, 223, 16, 16);
      }
      drawString(PoseStack, fontrenderer, this.getMessage(), this.getX() + 20 + 4, this.getY() + (this.height - 8) / 2, textColor);
    }
  }
