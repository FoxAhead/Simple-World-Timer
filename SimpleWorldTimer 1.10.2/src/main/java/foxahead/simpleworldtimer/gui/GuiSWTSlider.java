...
public class GuiSWTSlider extends GuiButton {
  ...
  protected void mouseDragged(Minecraft par1Minecraft, int par2, int par3) {
    if (this.enabled) {
      ...
      this.drawTexturedModalRect(this.xPosition + (this.sliderValue * (this.width - 8) / 100), this.yPosition, 0, 66, 4, 20);
      this.drawTexturedModalRect(this.xPosition + (this.sliderValue * (this.width - 8) / 100) + 4, this.yPosition, 196, 66, 4, 20);
    }
  }
  ...
  private void setSliderByMouse(int x) {
    int newSliderValue = MathHelper.clamp_int(Math.round((float) (x - (this.xPosition + 4)) * 100 / (this.width - 8)), 0, 100);
