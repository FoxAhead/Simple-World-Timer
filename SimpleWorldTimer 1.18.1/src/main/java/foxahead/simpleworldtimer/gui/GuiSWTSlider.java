...
public class GuiSWTSlider extends AbstractWidget {
  ...
  public GuiSWTSlider(int id, int x, int y, String text, int value, ISWTSliderObserver sliderObserver) {
    super(x, y, 200, 20, new TextComponent(text));
  ...
  @Override
  protected void renderBg(PoseStack PoseStack, Minecraft p_renderBg_1_, int p_renderBg_2_, int p_renderBg_3_) {
    p_renderBg_1_.getTextureManager().bindForSetup(WIDGETS_LOCATION);
    ...
    this.blit(PoseStack, this.x + (this.sliderValue * (this.width - 8) / 100), this.y, 0, 46 + i, 4, 20);
    this.blit(PoseStack, this.x + (this.sliderValue * (this.width - 8) / 100) + 4, this.y, 196, 46 + i, 4, 20);
  }
  ...
  private void setSliderDisplayString() {
    this.setMessage(new TextComponent(this.sliderText + String.valueOf(this.sliderValue)));
  }
