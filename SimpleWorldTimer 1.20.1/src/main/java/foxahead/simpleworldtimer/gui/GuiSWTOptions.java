...
public class GuiSWTOptions extends Screen implements ISWTSliderObserver {
  ...
  public void init() {
    ...
    checkBoxEnable   = this.addRenderableWidget(new GuiSWTCheckbox(xCoord,
                                                                   ...
                                                                   100,
                                                                   20,
                                                                   Component.translatable("options.swt.enable"),
                                                                   ConfigSWT.getEnable(),
                                                                   (onPress11) -> {
    ...
    checkBoxAutoHide = this.addRenderableWidget(new GuiSWTCheckbox(xCoord + 100,
                                                                   ...
                                                                   100,
                                                                   20,
                                                                   Component.translatable("options.swt.autoHide"),
    ...
    smallButtonPreset       = this.addRenderableWidget(new GuiSWTSmallButton(xCoord,
                                                                             ...
                                                                             176,
                                                                             20,
                                                                             Component.translatable("options.swt.preset"),
                                                                             ConfigSWT.getPresetList(),
                                                                             ConfigSWT.getPreset(),
                                                                             (onPress21) -> {
                                                                                                       this.actionPerformed(21);
                                                                                                     }));
    buttonCustom            = this.addRenderableWidget(Button.builder(Component.literal("C"), (onPress4) -> {
                              this.actionPerformed(4);
                            }).bounds(xCoord + 180, yCoord + yStep * 4, 20, 20).build());
    smallButtonClockType    = this.addRenderableWidget(new GuiSWTSmallButton(xCoord,
                                                                             yCoord + yStep * 5,
                                                                             75,
                                                                             20,
                                                                             CommonComponents.EMPTY,                                            // I18n.getString("options.swt.clockType"),
                                                                             ConfigSWT.getClockTypeList(),
                                                                             ConfigSWT.getClockType(),
                                                                             (onPress22) -> {
                                                                               this.actionPerformed(22);
                                                                             }));
    this.startDateTextField = new EditBox(this.font, xCoord + 150, yCoord + yStep * 5, 50, 20, CommonComponents.EMPTY);                         // 1
    startDateTextField.setResponder((onText1) -> {
      this.onTextField(1);
    });
    this.patternTextField1 = new EditBox(this.font, xCoord, yCoord + yStep * 6, 176, 20, CommonComponents.EMPTY); // 2
    patternTextField1.setResponder((onText1) -> {
      this.onTextField(2);
    });
    this.patternTextField2 = new EditBox(this.font, xCoord, yCoord + yStep * 7, 176, 20, CommonComponents.EMPTY); // 3
    ...
    buttonSwap = this.addRenderableWidget(Button.builder(Component.literal(")"), (onPress5) -> {
      this.actionPerformed(5);
    }).bounds(xCoord + 180, (int) (yCoord + yStep * 6.5F), 20, 20).build());
    this.addRenderableWidget(Button.builder(CommonComponents.GUI_DONE, (onPress0) -> {
      this.actionPerformed(0);
    }).bounds(xCoord, yCoord + yStep * 8, 200, 20).build());
    buttonStart = this.addRenderableWidget(Button.builder(Component.literal(">"), (onPress6) -> {
                  this.actionPerformed(6);
                }).bounds(xCoord + 102, yCoord + yStep * 5, 30, 20).build());
    buttonPause = this.addRenderableWidget(Button.builder(Component.literal("||"), (onPress7) -> {
                  this.actionPerformed(7);
                }).bounds(xCoord + 136, yCoord + yStep * 5, 30, 20).build());
    buttonStop  = this.addRenderableWidget(Button.builder(Component.literal("[]"), (onPress8) -> {
                  this.actionPerformed(8);
                }).bounds(xCoord + 170, yCoord + yStep * 5, 30, 20).build());
    updateButtons();
    initialized = true;
  }
  ...
  public boolean keyPressed(int par1, int par2, int par3) {
    if (super.keyPressed(par1, par2, par3)) {
      ...
      /*
       * try { ConfigSWT.setStartYear(Integer.parseInt(startDateTextField.getText())); } catch
       * (Exception e) { ConfigSWT.setStartYear(0); }
       * ConfigSWT.setPattern1(patternTextField1.getText());
       * ConfigSWT.setPattern2(patternTextField2.getText());
       */
      /*
       * if (par1 == GLFW.GLFW_KEY_TAB) { if (startDateTextField.isFocused()) { focus = 0; } else if
       * (patternTextField1.isFocused()) { focus = 1; } else if (patternTextField2.isFocused()) {
       * focus = 2; } focus = (focus + 1) % 3; startDateTextField.setFocused2(focus == 0);
       * patternTextField1.setFocused2(focus == 1); patternTextField2.setFocused2(focus == 2); }
       */
  ...
  public void render(GuiGraphics guiGraphics, int par1, int par2, float par3) {
    ...
    guiGraphics.drawCenteredString(fontRenderer,
                                   Component.translatable("options.swt.title"),
                                   this.width / 2,
                                   yCoord,
                                   COLOR1);
    if (smallButtonClockType.getIndex() == ConfigSWT.CLOCK_TYPE_MINECRAFT) {
      guiGraphics.drawCenteredString(fontRenderer,
                                     Component.translatable("options.swt.startYear"),
                                     xCoord + 120,
                                     yCoord + yStep * 5 + 6,
                                     COLOR2);
    }
    this.startDateTextField.render(guiGraphics, par1, par2, par3);
    this.patternTextField1.render(guiGraphics, par1, par2, par3);
    this.patternTextField2.render(guiGraphics, par1, par2, par3);
